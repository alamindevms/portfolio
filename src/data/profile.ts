export const profile = {
  name: "Al Amin Hossain",
  role: "Frontend Developer",
  positioning: "React · Next.js · TypeScript",
  location: "Dhaka, Bangladesh",
  timezone: "GMT+6",
  email: "alamindev.io@gmail.com",
  phone: "01922866947",
  phoneHref: "tel:+8801922866947",
  linkedin: "https://www.linkedin.com/in/alaminhossainpro",
  website: "https://alamindev.io",
  /** Single source of truth for every "Download CV" CTA on the site. Relative so it works from any deployment path. */
  cvUrl: "al-amin-hossain-cv.pdf",
  years: "4+",
  /** Alt text for the hero image in src/assets/hero-image.* — adjust to match the actual image. */
  heroImageAlt: "Al Amin Hossain — Frontend Developer",
} as const;
